...
static void *webrtc_output_create(obs_data_t *settings,
...
static bool webrtc_output_start(void *data)
{
	...
	obs_webrtc_output_connect(output->obsrtc, obs_service_get_url(service),
...
static void webrtc_output_data(void *data, struct encoder_packet *packet)
{
	...
	pthread_mutex_lock(&output->write_mutex);
	if (output->obsrtc) {
		if (!obs_webrtc_output_write(output->obsrtc, packet->data,
